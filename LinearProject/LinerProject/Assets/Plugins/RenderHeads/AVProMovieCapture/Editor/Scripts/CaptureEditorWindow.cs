	...
	public class CaptureEditorWindow : EditorWindow
	{
...
#if UNITY_EDITOR_OSX
			capture._forceVideoCodecIndex = capture._codecIndex = Mathf.Max(-1, _videoCodecIndex);
		...
		private void DrawConfigGUI_Capture()
		{
			...
			if (_sourceType == SourceType.Camera360)
			{
				...
				EditorGUILayout.PropertyField(_propSupportCameraRotation, new GUIContent("Camera Rotation"));
		...
		private void DrawConfigGUI_Encoding()
		{
				...
				_propOutputType.enumValueIndex == (int)CaptureBase.OutputType.ImageSequence)
			{
				...
				if (isImageSequence)
				{
					EditorUtils.EnumAsDropdown("Format", _propImageSequenceFormat, Utils.ImageSequenceFormatNames);
					GUILayout.Space(8f);
				}
