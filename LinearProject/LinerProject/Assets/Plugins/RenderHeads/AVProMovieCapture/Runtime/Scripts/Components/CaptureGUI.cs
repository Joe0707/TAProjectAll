		...
		private void MyWindow(int id)
		{
			...
			if (_movieCapture != null)
			{
				...
				else if (_movieCapture._outputType == CaptureBase.OutputType.ImageSequence)
				{
					...
					GUILayout.Label("Using Image Codec: " + _movieCapture._imageSequenceFormat);
					GUILayout.EndHorizontal();
					...
					if (_shownSection == 3)
					{
						...
						int newCodecIndex = GUILayout.SelectionGrid(-1, Utils.ImageSequenceFormatNames, 1);
						GUILayout.EndScrollView();
						if (newCodecIndex >= 0)
						{
							_movieCapture._imageSequenceFormat = (ImageSequenceFormat)newCodecIndex;
							_shownSection = -1;
						}
					}
				}
